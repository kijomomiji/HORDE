...
def train_mscn(seed, dataset, version, workload, params, sizelimit):
    ...
    print(str(dataset)+" "+str(version)+" "+str(workload)+f' {(time.time()-start_stmp)/60:.2f} mins'+"\n")
    with open("./mscn_time.txt","a+") as f:
        f.write(str(dataset)+" "+str(version)+" "+str(workload)+f' {(time.time()-start_stmp)/60:.2f} mins'+"\n")
