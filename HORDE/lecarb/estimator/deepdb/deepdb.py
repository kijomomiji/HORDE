...
def train_deepdb(seed, dataset, version, workload, params, sizelimit):
    ...
    model_file = model_path / f"{table.dataset}-{table.version}-spn_rdc{args.rdc_threshold}_ms{args.ratio_min_instance_slice}-{seed}.pkl"
    ...
    with open("./deepdb_time.txt","a+") as f:
        f.write(str(dataset)+" "+str(version)+" "+str(workload)+f' {dur_min:.2f} mins'+"\n")
...
def update_deepdb(seed: int, dataset: str, new_version: str, workload: str, params: Dict[str, Any], overwrite: bool) -> None:
    ...
    new_model_file = model_path / f"{new_table.dataset}-{new_table.version}-spn_rdc{args.rdc_threshold}_ms{args.ratio_min_instance_slice}-{seed}.pkl"
    ...
    with open("./deepdb_time.txt","a+") as file:
        file.write(f'Save model to {new_model_file} Time spent since start: {dur_min:.4f} mins\n')
    #     f.write(str(dataset)+" "+str(version)+" "+str(workload)+f' {dur_min:.2f} mins'+"\n")
