...
#     sample_inputs=[]
#     for query in query_vec:
#         sample_input=[1 for i in range(len(sample_data))]

#         not_none_attr=[]
#         for attr in range(len(query)):
#             if query[attr]!=[0]:
#                 not_none_attr.append(attr)

#         for s in range(len(sample_data)):
#             one_sample=sample_data[s]
#             for attr in not_none_attr:
#                 if query[attr][0]==1:
#                     if one_sample[attr]!=query[attr][1]:
#                         sample_input[s]=0
#                         break
#                 elif query[attr][0]==2:
#                     if one_sample[attr]<query[attr][1]:
...
#                         break
#                 elif query[attr][0]==4:
#                     if one_sample[attr]<query[attr][1] or one_sample[attr]>query[attr][2]:
#                         sample_input[s]=0
#                         break
...
def CE_plus_sample(dataset,version,params):
    time_for_BND_CN=0
    time_for_BND_CN_plus_T=0
    time_for_BND_CN_plus_AR=0
    ...
    # workload_data=load_data_from_pkl_file(dataset+"_"+version+"updated_workload.pkl")
    ...
    sample_number=args.num_samples
    ...
    time2=time.time()
    print("has spent ",time2-time1," seconds to get training samples")
    print_list.append("has spent "+str(time2-time1)+" seconds to get training samples")

    time1=time.time()
    valid_sample_inputs=sample_to_vector(valid_query_vec,sample_data)
    time2=time.time()
    print("has spent ",time2-time1," seconds to get valid samples")
    print_list.append("has spent "+str(time2-time1)+" seconds to get valid samples")
    ...
    time_for_BND_CN+=(time2-time1)
    

    addr="./lecarb/estimator/mine/samples_data/"+dataset+"_"+version+".pkl"
    with open(addr, 'wb') as f:
        pickle.dump([sample_data,sample_inputs,test_sample_inputs,valid_sample_inputs], f)
        print("the vec data has been stored in "+addr)
    ...
    model=SetConv(query_vec_for_classification.shape[2],query_vec_for_classification.shape[3],sample_number,hid_size=args.hid_units)
    report_model(model)
    loss_func=My_loss()
    lr=0.001
    ...
    # if False:
        ...
        end_time=time.time()
        print("has spent "+str(end_time-start_time)+" seconds to fill ACCT")
        print_list.append("has spent "+str(end_time-start_time)+" seconds to fill ACCT")
        with open(addr, 'wb') as f:
    ...
    for epoch in range(args.epochs):
        ...
        p_labels=np.round(valid_query_label.cpu().detach().numpy()*len(new_data))
    ...
    time_for_BND_CN+=inference_time
    ...
    with open(addr, 'rb') as f:
        [AR_inference_result,AR_inference_time] = pickle.load(f)
    for i in range(len(prediction)):
        if prediction[i]<thres:
            time_for_AR+=AR_inference_time[i]

    # print(time_for_AR)
    print_list.append(str(time_for_AR)+" ,if using AR tree to modify results")
    time_for_BND_CN_plus_AR=time_for_BND_CN+time_for_AR
    # return
    ...
    end_time=time.time()
    info="spent "+str(end_time-start_time)+" seconds using ACCT"
    time_for_BND_CN_plus_T=time_for_BND_CN+end_time-start_time
    print(info)
    ...
    for i in print_list:
        print(i)
    print("sample_number:",sample_number,"thres:",thres)
    print("average inference time:")
    print("raw BND_CN:",time_for_BND_CN*1000/len(test_query_label),'ms/query')
    print("BND_CN & ACCT:",time_for_BND_CN_plus_T*1000/len(test_query_label),'ms/query')
    print('BND_CN & AR tree:',time_for_BND_CN_plus_AR*1000/len(test_query_label),'ms/query')


def tree_inference(dataset,version):
