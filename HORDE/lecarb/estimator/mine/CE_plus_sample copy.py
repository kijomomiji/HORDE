...
class SetConv(nn.Module):
    def __init__(self,predicate_num,predicate_length,sample_number):
        super(SetConv, self).__init__()
        self.predicate_mlp1 = nn.Linear(predicate_length, 128)
        self.predicate_mlp2 = nn.Linear(128, 128)
        

        self.sample_mlp1 = nn.Linear(sample_number,sample_number*2)
        self.sample_mlp2 = nn.Linear(sample_number*2,128)
        
        self.out_mlp1 =nn.Linear(128*(predicate_num+1),512)
...
class My_loss(nn.Module):
    ...
    def forward(self, predications, labels, upper_bounds):
        ...
        return 0.8*torch.mean(torch.pow((predications - labels), 2)+0.2*torch.pow(torch.where(delta>0,delta,0),2))
        # return 0.8*torch.mean(torch.abs(predications - labels)+0.2*torch.pow(torch.where(delta>0,delta,0),2))
...
def CE_plus_sample(dataset,version):
    ...
    for query in query_vec:
        ...
        for s in range(len(sample_data)):
            for attr in range(len(sample_data[s])):
                if query[attr][0]==0:
                    continue
                elif query[attr][0]==1:
                    if sample_data[s][attr]!=query[attr][1]:
                        sample_input[s]=0
                        break
                elif query[attr][0]==2:
                    if sample_data[s][attr]<query[attr][1]:
                    ...
                    if sample_data[s][attr]<query[attr][1] or sample_data[s][attr]>query[attr][2]:
                        sample_input[s]=0
                        break
    ...
    model=SetConv(query_vec_for_classification.shape[2],query_vec_for_classification.shape[3],sample_number)
    ...
    for epoch in range(400):
        ...
        for i,(x,y,z) in enumerate(train_loader):
            ...
            loss = loss_func(out,batch_z,Variable(upper_bound))
        ...
        print("epoch ",epoch,":")
        out=model(query_vec_for_classification,sample_inputs)
        # for i in range(len(query_vec_for_classification)):
        #     if out[i]>least_C[i]:
        #         out[i]=least_C[i]
        ...
        #         print(query_label[th],out[th],upper_bound[th],q_error[th])
        print("Max:",np.max(q_error)," 99th:",np.percentile(q_error,99)," 95th:",np.percentile(q_error,95)," 90th:",np.percentile(q_error,90)," 50th:",np.percentile(q_error,50)," mean:",np.mean(q_error))
        # return

        prediction=np.round(out.detach().numpy()*len(new_data))
        p_labels=np.round(query_label.detach().numpy()*len(new_data))
